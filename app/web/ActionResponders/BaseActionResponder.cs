...
namespace LangBot.Web
{
    public abstract class BaseActionResponder : ISlackActionResponder
    {
        ...
        protected abstract Task<SlackMessage> Respond(SlackActionPayload payload, MemeMessage message);

        public async Task<SlackMessage> Respond(SlackActionPayload payload)
        {
            ...
            if (payload.Team.Id != message.TeamId) throw new SlackException("Invalid access. TeamId does not match.");
            if (payload.Channel.Id != message.ChannelId) throw new SlackException("Invalid access. ChannelId does not match.");
